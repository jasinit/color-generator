...
function generateHSL(hueOverride = null) {
    ...
    const h = hueOverride !== null ? (hueOverride + randomInt(-10, 10)) % 360 : randomInt(0, 360);
...
const harmonies = {
    ...
    'Complementary': (hueOverride) => {
        ...
        return [0, 0, 180, 180, 180].map((deg, i) => {
            ...
            if (i === 3) l -= 20;
            if (i === 4) l += 25;
            return hslToHex((base.h + deg) % 360, base.s, Math.min(95, Math.max(5, l)));
        });
    }
};
...
function renderPalette(hueOverride = null) {
    ...
    const existingCards = Array.from(elements.container.querySelectorAll('.swatch-card'));
...
function createCard(color, index) {
    ...
    card.style.animation = `cardEntrance 0.6s cubic-bezier(0.34, 1.56, 0.64, 1) forwards ${index * 0.1}s`;
...
// Dictionary
const moodDict = {
    'red': 0, 'fire': 10, 'passion': 350, 'anger': 0,
    'orange': 30, 'sunset': 25, 'warm': 35, 'autumn': 20,
    'yellow': 50, 'happy': 55, 'sun': 45, 'bright': 50,
    'green': 120, 'nature': 100, 'forest': 130, 'fresh': 110,
    'cyan': 180, 'teal': 170, 'beach': 180,
    'blue': 220, 'ocean': 210, 'calm': 200, 'sky': 200, 'sad': 230,
    'purple': 270, 'royal': 260, 'magic': 280, 'luxury': 270,
    'pink': 320, 'love': 330, 'candy': 310, 'flower': 300
};
...
function processChat() {
    ...
    setTimeout(() => {
        if (matchedHue !== null) {
            ...
            addMessage(`I love the vibe of "${keywordFound}"! Here's a palette inspired by that.`);
        } else {
            renderPalette(); // Random
            addMessage(`That's an interesting mood. Here's a random artistic palette for you!`);
        }
    }, 600);
}
...
function showToast(hex) {
    ...
    window.toastTimer = setTimeout(() => { elements.toast.classList.add('hidden'); }, 2000);
}
